...
<ul>
  ...
  <li>Ran everything against the actual loaded tables and checked the output row by row before writing up any findings, rather than trusting the query results blind</li>
</ul>
...
<p>I didn't just ask for a dataset and five queries and call it done. I checked the data generation logic made sense, actually ran and verified all five queries against the loaded tables myself, and worked through a real debugging problem instead of just accepting whatever came back. I think that's the more honest way to show this kind of workflow, not hiding that I used AI, but showing that I was still the one deciding what was correct.</p>
